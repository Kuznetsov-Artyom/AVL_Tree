...
template<typename T>
inline Node<T>* avlTree<T>::inserter(Node<T>*& node, const T& value, std::stack<Node<T>*>& way)
{
	if (node == nullptr)
	{
...
template<typename T>
inline const Node<T>* avlTree<T>::finder(const Node<T>* node, const T& value) const
{
	if (node == nullptr)
		return nullptr;
